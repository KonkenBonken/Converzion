...
import scss from './Swap.module.scss';

export default function SwapButton() {
  const path = usePathname()
  const newPath = path.split('/');
  newPath.push(newPath.pop() ?? '', newPath.pop() ?? '');

  return <a href={newPath.join('/')} className={scss.swapButton}><SwapIcon /></a>
}

export function SwapIcon() {
  return <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" role="button">
    <title>Swap</title>
